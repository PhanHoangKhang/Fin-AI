...
export const AiAnalysisCard: React.FC<AiAnalysisCardProps> = ({ news }) => {
  return (
    <section className="bg-[#2B3A1A] text-white p-8 shadow-2xl border-[#3D5226] border space-y-8">
      {/* 1. Header Báo cáo */}
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[#3D5226] pb-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[#3D5226]/30 border border-[#3D5226] p-4">
        <div className="text-center sm:text-left border-b sm:border-b-0 sm:border-r border-[#3D5226]/60 pb-3 sm:pb-0">
          <span className="text-[10px] uppercase font-bold text-slate-400 block mb-1">
            Vùng Mua Khuyến Nghị
          </span>
          <span className="text-sm font-extrabold text-[#9CB953]">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <article className="space-y-2 bg-[#3D5226]/30 p-5 border border-[#3D5226]/40">
        ...
        {/* Động lực chính  */}
        <article className="space-y-2 bg-[#3D5226]/40 p-5 border border-[#3D5226]">
          <h3 className="text-xs font-bold uppercase tracking-wider flex items-center gap-2">
             Động Lực
            Tăng Giá 
          </h3>
      ...
      <div className="space-y-3 pt-2">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
          <AcademicCapIcon className="w-4 h-4 text-[#9CB953]" /> Chi Tiết Chiến
          Lược Đầu Tư & Cơ Sở Luận Điểm
        </h3>
        <div className="grid md:grid-cols-3 gap-4">
          {/* Ngắn hạn */}
          <div className="bg-[#3D5226]/40 p-4 rounded-2xl border border-[#3D5226]/60 space-y-2">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-[#3D5226]/30 p-5 rounded-2xl border border-[#3D5226] space-y-2">
        ...
        <div className="bg-[#3D5226]/30 border border-[#3D5226]/50 p-5 rounded-2xl space-y-2">
      ...
      {news.keywords && news.keywords.length > 0 && (
        <footer className="pt-4 border-t border-[#3D5226]/80 flex items-center gap-3">
          <TagIcon className="w-4 h-4 text-slate-500 shrink-0" />
          <ul className="flex flex-wrap gap-2 text-xs">
            {news.keywords.map((kw: string, idx: number) => (
              <li
                key={idx}
                className="bg-[#3D5226]/80 text-slate-300 px-3 py-1 rounded-xl border border-[#3D5226]/60 font-medium"
              >
                #{kw}
              </li>
            ))}
